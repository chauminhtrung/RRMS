...
const AddTenantModal = ({ open, onClose }) => {
  ...
  const handleChange = (e) => {
    ...
    handleInputChange(e) // Nếu `handleInputChange` xử lý cập nhật trạng thái của `tenant`
  }
  const saveTenant = async (e) => {
    e.preventDefault()

    // Kiểm tra dữ liệu trước khi gửi
    if (!tenant.fullname || !tenant.phone || !tenant.CCCD) {
    ...
    // Định dạng form-data nếu có hình ảnh
    const formData = new FormData()
    formData.append('fullname', tenant.fullname)
    formData.append('phone', tenant.phone)
    formData.append('CCCD', tenant.CCCD)
    formData.append('birthday', tenant.birthday)
    formData.append('job', tenant.job)
    formData.append('address', tenant.address)

    if (tenant.front_photo) {
      formData.append('front_photo', tenant.front_photo)
    }
    if (tenant.back_photo) {
      formData.append('back_photo', tenant.back_photo)
    }
    ...
    try {
      ...
      const response = await axios.post('http://localhost:8080/tenant', formData, {
        headers: {
      ...
      if (response.status === 200) {
        setSnackbarMessage('Thêm thông tin khách thuê thành công!')
      ...
      setSnackbarMessage('Lỗi khi gửi dữ liệu. Vui lòng kiểm tra lại!')
      setSnackbarSeverity('error')
      setOpenSnackbar(true)
    } finally {
        ...
        <Box
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} sm={6}>
              ...
              <Grid item xs={12}>
                <RadioGroup row value={idType} onChange={handleChange}>
              ...
              <Grid item xs={12} sx={{ mt: 1 }}>
                <TextField label="CMND/CCCD" value={tenant.CCCD} onChange={handleInputChange} fullWidth />
              </Grid>
              <Grid item xs={12} sx={{ mt: 1 }}>
                <TextField label="Zalo của khách" value={tenant.phone} onChange={handleInputChange} fullWidth />
              </Grid>
              <Grid container spacing={2} sx={{ mt: 1 }}>
                <Grid item xs={12} sm={6}>
                  <TextField
              ...
              <Grid item xs={12} sx={{ mt: 1 }}>
                <TextField label="Nhập công việc" fullWidth value={tenant.job} onChange={handleInputChange} />
              </Grid>
              <Grid item xs={12} sx={{ mt: 2, display: 'flex', gap: 2 }}>
                <Grid item xs={12} sm={6}>
                  <TextField
                    label="Ngày cấp CMND/CCCD"
                    value={tenant.License_date}
                    onChange={handleInputChange}
                    type="date"
                    fullWidth
                    InputLabelProps={{ shrink: true }}
              ...
              <Grid item xs={12} sx={{ mt: 2, display: 'flex', gap: 2 }}>
                <Grid item xs={12} sm={6}>
                  <Box
                    ...
                    <Typography variant="body2" sx={{ mt: 1, color: '#9e9e9e' }}>
                      {tenant.front_photo
                        ? `Ảnh mặt trước CMND/CCCD: ${tenant.front_photo}`
                        : 'Chưa có ảnh mặt trước CMND/CCCD'}
                    </Typography>
                  </Box>
                </Grid>
                <Grid item xs={12} sm={6}>
                  <Box
                    ...
                    <Typography variant="body2" sx={{ mt: 1, color: '#9e9e9e' }}>
                      {tenant.back_photo
                        ? `Ảnh mặt sau CMND/CCCD: ${tenant.back_photo}`
                        : 'Chưa có ảnh mặt sau CMND/CCCD'}
                    </Typography>
                  </Box>
                </Grid>
              </Grid>
              ...
              <Box
                ...
                <Grid item xs={12} sx={{ display: 'flex', justifyContent: 'flex-end' }}>
                  ...
                  <Button
                    ...
                    {loading ? <CircularProgress size={24} /> : 'Thêm thông tin khách thuê'}
                  </Button>
                </Grid>
              </Box>
            </Grid>
