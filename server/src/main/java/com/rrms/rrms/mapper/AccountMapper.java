...
@Mapper(componentModel = "spring") //Mapper sử dụng MapStruct,Spring tự động phát hiện và quản lý mapper này như một Bean
public interface AccountMapper {
    ...
    @Mapping(target = "role", source = "account", qualifiedByName = "mapRole") // Thiết lập rằng trường "role" trong AccountResponse sẽ được lấy từ phương thức mapRole
    @Mapping(target = "permissions", source = "account", qualifiedByName = "mapPermissions") // Thiết lập rằng trường "permissions" sẽ được lấy từ phương thức mapPermissions
    AccountResponse toAccountResponse(Account account); // Phương thức nhận một Account và trả về đối tượng AccountResponse
    ...
    void updateAccount(@MappingTarget Account user, AccountRequest request); // Cập nhật user dựa trên thông tin từ AccountRequest
    ...
    @Named("mapPermissions") // Đánh dấu phương thức này với tên "mapPermissions" để có thể sử dụng trong các ánh xạ khác
