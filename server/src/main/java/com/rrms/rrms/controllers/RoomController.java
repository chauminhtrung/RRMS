...
public class RoomController {
    ...
    public ApiResponse<String> deleteRoom(@PathVariable("roomId") UUID roomId) {
        ...
        try {
            ...
            return ApiResponse.<String>builder().code(HttpStatus.OK.value()).message("Delete success").result(result).build();
        } catch (Exception e) {
            log.error("Delete room fail at roomId: {}", roomId);
            return ApiResponse.<String>builder().code(HttpStatus.BAD_REQUEST.value()).message("Delete fail").result(result).build();
        }
    }
}
